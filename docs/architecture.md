...
3. 应用优先使用用户选定的版本，其次使用最近一次健康检查成功的当前版本；两者都不存在时，才会通过配置的 registry 查询 `@deepseek-ai/dsh@latest` 的精确版本和完整性信息。
...
- DSH 版本选择与已安装版本目录。
- 更新检查频率：每次启动、每天、每周、每月或不检查；也可手动立即检查。检查到的新版本会先下载和校验，不会替换运行中的服务。
